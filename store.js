...
async function fetchApprovedByKeyword(keyword){
  /* No client (CDN blocked, offline) is a FAILED lookup, not an empty one, returning [] here showed "no one is listed" for keywords already sold. */
...
/* Every live keyword, deduped on the same normalization search uses, with how
   many companies hold it. The sample fixture never leaves the database here, fake companies must not appear as suggestions or on Browse. Cached for the
