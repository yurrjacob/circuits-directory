       ...
       logos bucket under covers/. */
{
  ...
  assert.ok(/const cover = isLogoUrl\(co\.cover_url\) \? co\.cover_url : ''/.test(pf) && /function coverHtml\(url, name\)/.test(pf)
...
assert.ok(/let tab = \(location\.hash \|\| ''\)\.replace\('#', ''\);[\s\S]{0,700}activateTab\(tab\)/.test(portalJsSrc), 'portal.js no longer opens the tab named in the hash (/portal#listings, /portal#seeking)');
